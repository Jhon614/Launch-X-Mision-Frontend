const fetchPokemon = async () => {
    ...
    const data = await fetch(url).then((res) => {
        if (res.status != "200") {
            ...
            pokeIdName.textContent = "Pokemon No Encontrado";
        }
        else {
            return res.json();
        }
...
const pokeTypeAbility = (pokeTypes, pokeAbilities) => {
    const pokemonTypes = [...(document.getElementsByClassName("pokemon-tipo"))];
    const pokemonAbilities = [...(document.getElementsByClassName("pokemon-poder"))];
